...
- **Benign:** Normal and safe network traffic  
- **Malicious:** Potential cyberattacks such as DDoS or Port Scanning
""")


# 1. DATA LOADING (SIMULATED DATA)

@st.cache_data
def load_data():
    """
    Generates a synthetic dataset mimicking CIC-IDS2017 network traffic.
    """
    np.random.seed(42)
    n_samples = 5000
...
st.write("Enter packet values to test whether traffic is malicious.")

c1, c2, c3, c4 = st.columns(4)

p_dur = c1.number_input("Flow Duration (ms)", 0, 100000, 500)
p_pkts = c2.number_input("Total Forward Packets", 0, 500, 100)
...
if st.button("Analyze Packet"):
    if 'model' in st.session_state:
        ...
        if prediction[0] == 1:
            ...
            st.write("**Reason:** High packet activity with suspicious duration.")
        else:
